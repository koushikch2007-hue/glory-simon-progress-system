...
export default function ReportForm({ user, onCancel, onSubmitSuccess, backendUrl }) {
  ...
  const handleRemovePhoto = (index) => {
    ...
    if (photoToRemove.url.startsWith('blob:')) {
      URL.revokeObjectURL(photoToRemove.url); // free memory
    }
        ...
        <div className="form-grid">
          ...
          <div className="form-group form-grid-full">
            ...
            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>
              Note: Adding at least one issue will automatically update the status of this report to <span style={{ fontWeight: 600, color: 'var(--color-danger)' }}>Action Required</span>.
            </p>
          </div>
